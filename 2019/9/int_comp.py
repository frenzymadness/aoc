class IntComputer:
    ...
    def load_param(self, position, mode):
        if mode == 0:
            ...
            return self.program[self.relative_base + self.program[position]]
        return None
    ...
    def store_result(self, position, mode, result):
        if mode == 0:
            ...
            self.program[self.relative_base + self.program[position]] = result
    ...
    def run(self):
        while True:
            ...
            # print(instruction, self.pointer, self.program, self.input, self.relative_base)
            ...
            if instruction == 9:
                ...
                self.relative_base += self.load_param(self.pointer+1, m1)
                self.pointer += 2
                continue

if __name__ == "__main__":
    computer = IntComputer("input", [2])
    print(computer.run())
